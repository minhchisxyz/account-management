...
class TransactionAPI {
  ...
  static Future<List<GroupedByDateTransaction>> fetchAllTransactionsGroupedByDate(int year, int month) async {
    final response = await http.get(Uri.parse('$baseUrl/years/$year/months/$month?grouped=true'), headers: _headers);

    if (response.statusCode == 200) {
      final List<dynamic> data = json.decode(response.body);
      return data.map((json) => GroupedByDateTransaction.fromJson(json)).toList();
    } else {
      throw Exception('Failed to load grouped transactions for $year-$month');
    }
  }
}
