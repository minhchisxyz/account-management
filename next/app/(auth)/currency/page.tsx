...
import CurrencyLoadingPage, {CurrencyGraphLoading} from "@/components/currency-loading-page";
...
export default function CurrencyContentPage() {
  ...
  const handleFilterClick = async (filter: number) => {
  ...
  if (loading) return <CurrencyLoadingPage/>
  return (
      <div className={"flex flex-col items-center justify-start w-full max-h-screen py-4 px-4 md:px-10 gap-10"}>
        ...
        <div className={"w-full max-w-5xl mx-auto overflow-x-auto"}>
          ...
          {
            graphLoading ? <CurrencyGraphLoading/> : <Graph
